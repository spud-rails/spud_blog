class BlogController < ApplicationController
  ...
  def filter
    if !params[:category_url_name].blank? && !params[:archive_date].blank?
      redirect_to spud_blog.blog_category_archive_path(params[:category_url_name], params[:archive_date])
    elsif !params[:category_url_name].blank?
      redirect_to spud_blog.blog_category_path(params[:category_url_name])
    elsif !params[:archive_date].blank?
      redirect_to spud_blog.blog_archive_path(params[:archive_date])
    else
      redirect_to spud_blog.blog_path
    end
  end
  ...
  def create_comment
    ...
    if @post.blank?
      ...
      redirect_to spud_blog.blog_path and return false
    end
    ...
    @comment.permalink = spud_blog.blog_post_url(@post.url_name)
    flash[:notice] = 'Your comment has been posted.' if @comment.save
    respond_with @comment do |format|
    	format.html { redirect_to spud_blog.blog_post_path(@post.url_name, :anchor => 'spud_post_comment_form') }
    end
  end
